...
contract('Election', function(accounts){
    ...
    it("votes from unauthorized users should not count", function(){
        ...
        return Election.deployed().then(function(instance){
            ...
            assert.equal(preVoteCount, firstCandidateResult[1].toNumber(), "unauthor user vote was counted");
        });
    });

    it("non-owners cannot authorize voters", function(){
        return Election.deployed().then(function(){
            return instance.authorize(accounts[i], {from: accounts[i]});
        }).then(function(){
            assert(false,"non-owner cannot call authorize");
        }).catch(function(){
            assert(true, "expected error when non-owner call authorize");
        });
    });

    it("authorized votes should be counter", function(){
        var electionInstance;
        var preVoteCount;
        return Election.deployed().then(function(instance){
            electionInstance = instance;
    ...
    it("authorized users can only vote once", function(){
        return Election.deployed().then(function(instance){
            ...
            assert(true, "expected error when user votes twice");
        });
    });
});
