App = {
    ...
    bindEvents:function(){
        ...
        $(".auth").on('click', function(){
            var voter = $(this).attr("data-index");
            App.authVoter(voter);
        })
    },

    authVoter: function(voter){
        console.log("App.authVoter()"+voter);
        console.log({from: web3.eth.accounts[0]})
        App.contracts.Election.deployed().then(function(instance){
           console.log("get acc"+web3.eth.getAccounts)

            /*return instance.authorize({
               from: web3.eth.accounts[voter]
